...
import { Download, Folder, File, Trash2, Upload, RefreshCw } from "lucide-react";
...
export function FileBrowser({ boxId }: { boxId?: string }) {
  ...
  return <div className="panel">
    <div className="toolbar">
      ...
      <button onClick={async () => { const name = prompt("目录名"); if (name) { await api.mkdir(boxId, `${path === "." ? "" : `${path}/`}${name}`); await load(); } }}>新建目录</button>
    </div>
